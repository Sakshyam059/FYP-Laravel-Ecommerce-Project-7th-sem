<div x-data="{ modelOpen: false }" id="subcategoryModal">
    ...
    <div x-show="modelOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen px-4 text-center md:items-center sm:block sm:p-0">
            ...
            >
                ...
                <form method="post" class="mt-5" id="subcategoryForm">
                    @csrf
                    <div class="mb-2 space-y-2">
                        <div>
                            <label for="">Subcategory Name</label>
                            <input type="text" name="subcategory_name" value="{{ old('subcategory_name') }}"
                                placeholder="Enter subcategory name" class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40">
                            <span class="hidden text-xs text-red-600" id="subcategory_name_error"></span>
                    
                            </div>
                    </div>
                    <div class="mb-2 space-y-2 " >
                        <label for="">Category</label>
                        <select class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40" name="category_id" >
                            <option selected disabled>Choose a category</option>
                            @foreach ($categories as $category)
                            <option value="{{ $category->id }}">{{ $category->category_name }}</option>
                            @endforeach
                        </select>
                        <span class="hidden text-xs text-red-600" id="category_id_error"></span>

                    </div>
                    <div>
                        <label for="description" class="block pb-3 text-sm text-gray-700 capitalize dark:text-gray-200">Description</label>
                        <textarea name="description" class="w-full text-sm text-gray-700 capitalize rounded dark:text-gray-200" id="description"  rows="3"></textarea>    
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            ...
                            $('#subcategoryModal').attr('x-data', '{modelOpen:false}')
                        }
                    },

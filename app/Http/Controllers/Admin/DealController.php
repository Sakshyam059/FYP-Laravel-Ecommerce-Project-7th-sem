...
class DealController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            ->addIndexColumn()
                ->editColumn('select_deals', function ($row) {
                    return '<input class="mx-3 rounded select-all lg:mx-1" type="checkbox" name="deals[]" value="' . $row->id . '"/>';
                ...
                $delete = route('admin.product.brand.destroy', $row->id);
    ...
    return $btn;
                ...
                ->rawColumns(['select_deals','status', 'action'])
                ->make(true);
        }
    ...
    public function store(DealPostRequest $request)
    {
        try {
            ...
            Deal::create($validator);
            return response()->json(['status' => 200, 'success' => true, 'message' => 'Deal Created successfully']);
        } catch (\Exception $e) {
            dd($e->getMessage());
            return redirect()->back()->with('error', 'Failed to create  deal. Please try again.');
        }

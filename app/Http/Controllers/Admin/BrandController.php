...
class BrandController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            ->addIndexColumn()
                ->editColumn('select_brands', function ($row) {
                    return '<input class="mx-3 rounded select-all lg:mx-1" type="checkbox" name="brands[]" value="' . $row->id . '"/>';
    ...
    return $btn;
                ...
                ->rawColumns(['select_brands','status', 'action'])
                ->make(true);
        }
     ...
     */
    public function update(BrandPostRequest $request, Brand $brand)
    {
        try {
            $validator = $request->validated();
            $validator['slug'] = Str::slug($request->brand_name);
            $brand->update($validator);
            return to_route('admin.product.brand.index')->with('success', 'Brand Updated Successfully.');
